...
read.csv(file = "23topics.csv") -> twenty3_topic
#combine the topic 
...
best_topic_23_15<- parLapply(cl, 1:5, topic_fun1, topic = 0) #only 23topics and 15topics
stopCluster(cl)

mean(unlist(best_topic_23_15))

#---------------------------------

topic_fun2 <- function(i, topic){
...
best_topic_7_15<- parLapply(cl, 1:5, topic_fun2, topic = 0) #only 7topics and 15topics
stopCluster(cl)

mean(unlist(best_topic_7_15))
rowMeans(cv_tab.4)
                           
#---------------------------------------------------------


topic_fun3 <- function(i, topic){
...
best_topic_7_23<- parLapply(cl, 1:5, topic_fun3, topic = 0) #only 7topics and 23topics
stopCluster(cl)

mean(unlist(best_topic_7_23))
mean(unlist(best_topic_7_15))
mean(unlist(best_topic_23_15))
mean(unlist(best_topic_all))
...
#----------------------Boruta
boruta_df <- read.csv(file = "data/processed_train_df_4.csv", stringsAsFactors = FALSE)

boruta_df[,-1] -> boruta_df
colnames(boruta_df)[1] <- "emails.V1"

cl <- makeCluster(4) 
clusterExport(cl,"cv_idx")
clusterExport(cl,"boruta_df")
clusterExport(cl, "mod")
clusterEvalQ(cl, library(randomForest))

boruta_output <- parLapply(cl, 1:5, rf, df = boruta_df)
